...
class Debug extends CI_Controller
{
    ...
    public function test_spot_price()
    {
        ...
        if ($spot_price) {
            $this->session->set_flashdata('success', 'Spot price for ' . $symbol . ': $' . number_format($spot_price->price, 2));
        } else {
    ...
    public function test_futures_price()
    {
        ...
        if ($futures_price) {
            $this->session->set_flashdata('success', 'Futures price for ' . $symbol . ': $' . number_format($futures_price->price, 2));
        } else {
